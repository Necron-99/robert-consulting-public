...
Write-Host "🔧 Enabling HTTPS and Custom Domains for robertconsulting.net" -ForegroundColor Green
...
Write-Host "✅ Certificate is validated! Proceeding with HTTPS and custom domain setup..." -ForegroundColor Green

Write-Host ""
Write-Host "🔧 Step 2: Update CloudFront configuration for HTTPS and custom domains..." -ForegroundColor Yellow
...
$content = Get-Content infrastructure.tf
$content = $content -replace 'acm_certificate_arn = aws_acm_certificate.wildcard.arn', "acm_certificate_arn = aws_acm_certificate.wildcard.arn`n    ssl_support_method       = `"sni-only`"`n    minimum_protocol_version = `"TLSv1.2_2021`""
$content | Set-Content infrastructure.tf
(Get-Content infrastructure.tf) -replace 'viewer_protocol_policy = "allow-all"', 'viewer_protocol_policy = "redirect-to-https"' | Set-Content infrastructure.tf
...
Write-Host ""
Write-Host "✅ HTTPS and custom domains enabled!" -ForegroundColor Green
Write-Host ""
